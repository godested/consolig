...
if __name__ == '__main__':
    ...
    for config in STYLE_CONFIGS:
        ...
        if path.exists(source_file_path) and path.exists(input_file_path):
            ...
            opentype_file = Path(SOURCE_DIR / 'opentype.json')
            with open(opentype_file, 'r') as file:
                opentype_config = json.load(file)
            output_glyph_names = output_font.getGlyphNames()
            classes_string = '@NotSpace = [\n'
            for glyph_name in output_glyph_names:
                if glyph_name != 'space':
                    classes_string += f'  {glyph_name}\n'
            classes_string += '];\n'
            for file_name in opentype_config['classes']:
                file_path = Path(OPENTYPE_DIR / 'classes' / file_name)
                if file_path.is_file():
                    with open(file_path, 'r') as file:
                        classes_string += file.read() + '\n'
            prefix_file = Path(OPENTYPE_DIR / 'prefix.fea')
            prefix_string = ''
            with open(prefix_file, 'r') as file:
                prefix_string += file.read() + '\n'
            aalt_string = 'feature aalt {\n'
            features_string = ''
            for file_name in opentype_config['features']:
                file_path = Path(OPENTYPE_DIR / 'features' / file_name)
                if file_path.is_file():
                    with open(file_path, 'r') as file:
                        feature_name = path.splitext(file_name)[0]
                        aalt_string += f'  feature {feature_name};\n'
                        features_string += f'feature {feature_name} {{\n'
                        feature_text_lines = file.read().splitlines(True)
                        for feature_text_line in feature_text_lines:
                            if feature_text_line.isspace():
                                features_string += feature_text_line
                            else:
